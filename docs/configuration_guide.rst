...
|-----------|------|-------------|-------------|
| `static_input_dim` | int | ≥ 0 | Static feature dimension |
| `dynamic_input_dim` | int | ≥ 0 | Dynamic feature dimension |
| `future_input_dim` | int | ≥ 0 | Future feature dimension |
| `output_dim` | int | ≥ 1 | Number of output variables |
| `forecast_horizon` | int | ≥ 1 | Forecast horizon |
...
|-----------|------|---------|-------|-------------|
| `embed_dim` | int | 32 | [8, 512] | Embedding dimension |
| `hidden_units` | int | 64 | [16, 1024] | Dense hidden units |
| `lstm_units` | int | 64 | [16, 1024] | LSTM units |
| `attention_units` | int | 64 | [16, 1024] | Attention dimension |
...
|-----------|------|---------|-------------|
| `dropout_rate` | float | 0.2 | Dropout probability [0-1] |
...
|-----------|------|---------|---------|-------------|
| `use_vsn` | bool | True | - | Enable Variable Selection Network |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `quantiles` | list[float] | None | Quantiles for uncertainty |
...
Use `architecture_config` dictionary for structural choices:
...
|------|---------|----------|
| `cross` | Encoder-decoder interaction | Always recommended |
| `hierarchical` | Multi-level temporal patterns | Seasonal data |
| `memory` | Historical pattern retrieval | Long-range dependencies |
| `attention` | Generic self-attention | Transformer mode |
