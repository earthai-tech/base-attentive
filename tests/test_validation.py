...
def fake_runtime(validation_module, monkeypatch):
    ...
    monkeypatch.setattr(
        validation_module, "_has_runtime", lambda: True
    )
    monkeypatch.setattr(
        validation_module, "KERAS_DEPS", _FakeKerasDeps()
    )
    return validation_module
...
class TestValidationModule:
    ...
    def test_validate_model_inputs_none_returns_empty_triplet(
        ...
        static, dynamic, future = (
            validation_module.validate_model_inputs(None)
        )
    ...
    def test_validate_model_inputs_wraps_conversion_errors(
        ...
        with pytest.raises(
            ValueError, match="Could not convert input"
        ):
            fake_runtime.validate_model_inputs("__boom__")

    def test_maybe_reduce_quantiles_bh_passthrough(
        self, fake_runtime
    ):
    ...
    def test_maybe_reduce_quantiles_bh_reduction_3d(
        self, fake_runtime
    ):
    ...
    def test_validate_model_inputs_verbose_logs_shapes(
        self, fake_runtime
    ):
    ...
    def test_maybe_reduce_quantiles_bh_rank4_callable(
        self, fake_runtime
    ):
    ...
    def test_maybe_reduce_quantiles_bh_3d_callable(
        self, fake_runtime
    ):
