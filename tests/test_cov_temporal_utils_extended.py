...
from base_attentive.components._temporal_utils import (  # noqa: E402
    aggregate_multiscale,
    aggregate_multiscale_on_3d,
    aggregate_time_window_output,
)
...
def uniform_seq_list():
    ...
    return [np.random.randn(B, T, U).astype(np.float32) for _ in range(3)]
...
class TestAggregateMultiscale:
    ...
    def test_mode_average(self, seq_list):
        result = _to_numpy(aggregate_multiscale(seq_list, mode="average"))
        assert result.shape == (B, U * 3)

    def test_mode_sum(self, seq_list):
        result = _to_numpy(aggregate_multiscale(seq_list, mode="sum"))
        assert result.shape == (B, U * 3)

    def test_mode_flatten(self, uniform_seq_list):
        result = _to_numpy(aggregate_multiscale(uniform_seq_list, mode="flatten"))
        assert result.shape[0] == B
        assert result.ndim == 2

    def test_mode_concat(self, uniform_seq_list):
        result = _to_numpy(aggregate_multiscale(uniform_seq_list, mode="concat"))
        assert result.shape == (B, U * 3)

    def test_mode_last(self, seq_list):
        result = _to_numpy(aggregate_multiscale(seq_list, mode="last"))
        assert result.shape == (B, U * 3)

    def test_mode_auto(self, seq_list):
        result = _to_numpy(aggregate_multiscale(seq_list, mode="auto"))
        assert result.shape == (B, U * 3)

    def test_single_scale(self):
        single = [np.random.randn(B, T, U).astype(np.float32)]
        result = _to_numpy(aggregate_multiscale(single, mode="last"))
...
class TestAggregateMultiscaleOn3D:
    def test_non_list_passthrough(self):
        tensor = np.random.randn(B, U * 3).astype(np.float32)
        result = _to_numpy(aggregate_multiscale_on_3d(tensor, mode="last"))
        assert result.shape == (B, U * 3)
    ...
    def test_mode_last(self, seq_list):
        result = _to_numpy(aggregate_multiscale_on_3d(seq_list, mode="last"))
        assert result.shape == (B, U * 3)

    def test_mode_auto(self, seq_list):
        result = _to_numpy(aggregate_multiscale_on_3d(seq_list, mode="auto"))
        assert result.shape == (B, U * 3)

    def test_mode_average(self, seq_list):
        result = _to_numpy(aggregate_multiscale_on_3d(seq_list, mode="average"))
        assert result.shape == (B, U * 3)

    def test_mode_sum(self, seq_list):
        result = _to_numpy(aggregate_multiscale_on_3d(seq_list, mode="sum"))
        assert result.shape == (B, U * 3)

    def test_mode_flatten(self, uniform_seq_list):
        result = _to_numpy(aggregate_multiscale_on_3d(uniform_seq_list, mode="flatten"))
        assert result.shape[0] == B
        assert result.ndim == 2
    ...
    def test_mode_concat_3d_output(self, seq_list):
        ...
        tensors = [keras.ops.convert_to_tensor(t) for t in seq_list]
        result = _to_numpy(aggregate_multiscale_on_3d(tensors, mode="concat"))
    ...
    def test_mode_concat_non_3d_raises(self):
        import keras
        bad = [keras.ops.convert_to_tensor(np.random.randn(B, U).astype(np.float32))]
...
class TestAggregateTimeWindowOutput:
    ...
    def test_mode_last(self, twx):
        result = _to_numpy(aggregate_time_window_output(twx, mode="last"))
        assert result.shape == (B, U)

    def test_mode_average(self, twx):
        result = _to_numpy(aggregate_time_window_output(twx, mode="average"))
        assert result.shape == (B, U)

    def test_mode_flatten(self, twx):
        result = _to_numpy(aggregate_time_window_output(twx, mode="flatten"))
        assert result.shape == (B, T * U)

    def test_mode_none_defaults_to_flatten(self, twx):
        result = _to_numpy(aggregate_time_window_output(twx, mode=None))
        assert result.shape == (B, T * U)

    def test_invalid_mode_raises(self, twx):
        with pytest.raises(ValueError, match="Unsupported mode"):
            aggregate_time_window_output(twx, mode="unknown_mode")
