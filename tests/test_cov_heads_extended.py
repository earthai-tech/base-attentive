...
class TestGaussianHead:
    ...
    def test_3d_input(self):
        head = GaussianHead(output_dim=2)
        features = np.random.randn(4, 10, 16).astype(np.float32)
        out = head(features)
    ...
    def test_get_config(self):
        cfg = GaussianHead(output_dim=3, min_scale=1e-3).get_config()
        assert cfg["output_dim"] == 3
        assert cfg["min_scale"] == pytest.approx(1e-3)

    def test_from_config(self):
        orig = GaussianHead(output_dim=2, forecast_horizon=5)
...
class TestMixtureDensityHead:
    def test_basic_call(self):
        head = MixtureDensityHead(output_dim=1, num_components=3)
        features = np.random.randn(4, 16).astype(np.float32)
        out = head(features)
        assert "weights" in out and "means" in out and "scales" in out
    ...
    def test_single_component(self):
        head = MixtureDensityHead(output_dim=1, num_components=1)
        features = np.random.randn(2, 8).astype(np.float32)
        out = head(features)
        assert out is not None

    def test_multi_output_dim(self):
        head = MixtureDensityHead(output_dim=2, num_components=2)
        features = np.random.randn(4, 16).astype(np.float32)
        out = head(features)
    ...
    def test_invalid_num_components_raises(self):
        with pytest.raises(ValueError):
            MixtureDensityHead(output_dim=1, num_components=0)

    def test_get_config(self):
        cfg = MixtureDensityHead(output_dim=1, num_components=4).get_config()
        assert cfg["num_components"] == 4

    def test_from_config(self):
        orig = MixtureDensityHead(output_dim=2, num_components=3)
        clone = MixtureDensityHead.from_config(orig.get_config())
...
class TestPointForecastHead:
    def test_2d_input(self):
        head = PointForecastHead(output_dim=1)
        features = np.random.randn(4, 16).astype(np.float32)
        out = _to_numpy(head(features))
        assert out.shape == (4, 1)

    def test_3d_input(self):
        head = PointForecastHead(output_dim=3)
        features = np.random.randn(4, 10, 16).astype(np.float32)
        out = _to_numpy(head(features))
        assert out.shape[-1] == 3

    def test_get_config(self):
        cfg = PointForecastHead(output_dim=2, forecast_horizon=7).get_config()
        assert cfg["output_dim"] == 2
        assert cfg["forecast_horizon"] == 7

    def test_from_config(self):
        orig = PointForecastHead(output_dim=4)
        clone = PointForecastHead.from_config(orig.get_config())
...
class TestQuantileHead:
    def test_output_shape_2d(self):
        head = QuantileHead(quantiles=[0.1, 0.5, 0.9], output_dim=1)
        features = np.random.randn(4, 16).astype(np.float32)
        out = _to_numpy(head(features))
    ...
    def test_output_shape_3d(self):
        head = QuantileHead(quantiles=[0.1, 0.9], output_dim=2)
        features = np.random.randn(4, 10, 16).astype(np.float32)
        out = _to_numpy(head(features))
    ...
    def test_empty_quantiles_raises(self):
        with pytest.raises(ValueError):
            QuantileHead(quantiles=[], output_dim=1)

    def test_get_config(self):
        cfg = QuantileHead(quantiles=[0.1, 0.5, 0.9], output_dim=1).get_config()
        assert cfg["quantiles"] == [0.1, 0.5, 0.9]

    def test_from_config(self):
        orig = QuantileHead(quantiles=[0.25, 0.75], output_dim=2)
...
class TestCombinedHeadLoss:
    ...
    def test_custom_heads_sum(self):
        from base_attentive.components.losses import AnomalyLoss
...
class TestQuantileDistributionModeling:
    ...
    def test_without_quantiles(self):
        qdm = QuantileDistributionModeling(quantiles=None, output_dim=2)
        features = np.random.randn(4, 10, 16).astype(np.float32)
        out = _to_numpy(qdm(features))
        assert out.shape[-1] == 2

    def test_auto_quantiles(self):
        qdm = QuantileDistributionModeling(quantiles="auto", output_dim=1)
        assert qdm.quantiles == [0.1, 0.5, 0.9]

    def test_get_config(self):
        cfg = QuantileDistributionModeling(
            quantiles=[0.1, 0.9], output_dim=1
        ).get_config()
        assert cfg["quantiles"] == [0.1, 0.9]

    def test_from_config(self):
        orig = QuantileDistributionModeling(quantiles=[0.5], output_dim=3)
        clone = QuantileDistributionModeling.from_config(orig.get_config())
