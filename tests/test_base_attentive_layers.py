...
from base_attentive.config.legacy_adapter import normalize_legacy_architecture_spec
...
def test_base_attentive_facade_preserves_legacy_arguments(patched_v2_init):
    model = BaseAttentive(
        static_input_dim=0,
        dynamic_input_dim=3,
        future_input_dim=2,
...
def test_base_attentive_facade_quantile_head_type_and_horizon(patched_v2_init):
    model = BaseAttentive(
        static_input_dim=1,
        dynamic_input_dim=1,
        future_input_dim=1,
