"""JAX-backend V2 builders implemented with Keras 3 primitives.

For Keras 3 multi-backend support, the JAX backend should return Keras
layers and callables that execute on the active JAX runtime, rather
than placeholder pure functions disconnected from Keras tracking.
"""
...
@register_keras_serializable(
    ...
    name="JaxTemporalSelfAttentionEncoder",
)
class _JaxTemporalSelfAttentionEncoder(Layer):
    """Keras multi-backend temporal encoder for the JAX backend."""
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE, name="JaxMeanPool"
)
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE, name="JaxLastPool"
)
class _LastPool(Layer):
    def __init__(self, *, axis=1, name=None, **kwargs):
        if axis != 1:
            raise ValueError(
                "jax last-pool currently supports axis=1 only."
            )
        super().__init__(
            name=name, **_clean_builder_kwargs(kwargs)
        )
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE, name="JaxConcatFusion"
)
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE,
    name="JaxCrossAttention",
)
class _JaxCrossAttention(Layer):
    def __init__(
        self,
        *,
        units: int,
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE,
    name="JaxHierarchicalAttention",
)
class _JaxHierarchicalAttention(Layer):
    def __init__(
        self,
        *,
        units: int,
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE,
    name="JaxMemoryAttention",
)
class _JaxMemoryAttention(Layer):
    """Lightweight learned-memory block for the JAX backend.

    Keras-on-JAX currently behaves much more reliably here with a simple
    learned memory summary than with a secondary multi-head attention call
    against a tiled memory bank. The layer still exposes a trainable memory
    bank and a query-conditioned gate, but avoids the full JAX MHA edge that
    previously stalled the end-to-end V2 forward path.
    """
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE,
    name="JaxMultiResolutionAttentionFusion",
)
class _JaxMultiResolutionAttentionFusion(Layer):
    def __init__(
        self,
        *,
        units: int,
    ...
    def call(self, inputs, training: bool = False):  # noqa: ARG002, FBT002
        encoded = self.input_dense(inputs)
        summary = mean_op(encoded, axis=1, keepdims=True)
        gate = self.gate_dense(encoded)
        fused = encoded + gate * summary
        return self.output_dense(fused)
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE,
    name="JaxMultiHorizonHead",
)
class _JaxMultiHorizonHead(Layer):
    def __init__(
        self,
        *,
        output_dim: int,
...
@register_keras_serializable(
    SERIALIZATION_PACKAGE,
    name="JaxQuantileDistributionHead",
)
class _JaxQuantileDistributionHead(Layer):
    def __init__(
        self,
        *,
...
def _build_jax_static_processor(**kwargs):
    _ensure_jax()
...
def _build_jax_dynamic_processor(**kwargs):
    _ensure_jax()
    return _delegate_generic(
        _build_generic_feature_processor, **kwargs
    )
...
def _build_jax_future_processor(**kwargs):
    _ensure_jax()
    return _delegate_generic(
        _build_generic_feature_processor, **kwargs
    )
...
def _build_jax_positional_encoding(**kwargs):
    _ensure_jax()
    return _delegate_generic(
...
def _build_jax_hybrid_multiscale_encoder(**kwargs):
    _ensure_jax()
    return _delegate_generic(
...
def _build_jax_dynamic_window(**kwargs):
    _ensure_jax()
    return _delegate_generic(
...
def _build_jax_cross_attention(
    *,
    context=None,
    spec=None,
    units: int | None = None,
...
def _build_jax_hierarchical_attention(
    *,
    context=None,
    spec=None,
    units: int | None = None,
...
def _build_jax_memory_attention(
    *,
    context=None,
    spec=None,
    units: int | None = None,
...
def _build_jax_multi_resolution_attention_fusion(
    *,
    context=None,
    spec=None,
    units: int | None = None,
...
def _build_jax_flatten_pool(**kwargs):
    _ensure_jax()
    return _delegate_generic(
...
def _build_jax_multi_horizon_head(
    *,
    context=None,
    spec=None,
...
def _build_jax_quantile_distribution_head(
    *,
    context=None,
    spec=None,
    output_dim: int | None = None,
    ...
    **kwargs,
    ...
    active_horizon = int(getattr(spec, "forecast_horizon", 1))
    return _JaxQuantileDistributionHead(
...
def ensure_jax_v2_registered(registry=None) -> None:
    ...
    registrations = [
        ("projection.dense", _build_jax_dense_projection),
        ("projection.static", _build_jax_dense_projection),
        ("projection.dynamic", _build_jax_dense_projection),
        ("projection.future", _build_jax_dense_projection),
        ("projection.hidden", _build_jax_dense_projection),
        (
            "feature.static_processor",
            _build_jax_static_processor,
        ),
        (
            "feature.dynamic_processor",
            _build_jax_dynamic_processor,
        ),
        (
            "feature.future_processor",
            _build_jax_future_processor,
        ),
        (
            "embedding.positional",
            _build_jax_positional_encoding,
        ),
        (
            "encoder.hybrid_multiscale",
            _build_jax_hybrid_multiscale_encoder,
        ),
        (
            "encoder.temporal_self_attention",
            _build_jax_temporal_self_attention_encoder,
        ),
        ("encoder.dynamic_window", _build_jax_dynamic_window),
        (
            "decoder.cross_attention",
            _build_jax_cross_attention,
        ),
        (
            "decoder.hierarchical_attention",
            _build_jax_hierarchical_attention,
        ),
        (
            "decoder.memory_attention",
            _build_jax_memory_attention,
        ),
        (
            "fusion.multi_resolution_attention",
            _build_jax_multi_resolution_attention_fusion,
        ),
        ("pool.mean", _build_jax_mean_pool),
        ("pool.last", _build_jax_last_pool),
        ("pool.final_last", _build_jax_last_pool),
        ("pool.final_mean", _build_jax_mean_pool),
        ("pool.final_flatten", _build_jax_flatten_pool),
        ("fusion.concat", _build_jax_concat_fusion),
        ("head.multi_horizon", _build_jax_multi_horizon_head),
        (
            "head.quantile_distribution",
            _build_jax_quantile_distribution_head,
        ),
        (
            "head.point_forecast",
            _build_jax_point_forecast_head,
        ),
        ("head.quantile_forecast", _build_jax_quantile_head),
    ]
    for key, builder in registrations:
        registry.register(key, builder, backend="jax")
