...
def create_causal_mask(size: int | Tensor) -> Tensor:
    ...
    idxs = range(size)  # shape: [size]

    # Compare row < col for every pair (i,j)
    #   row_idxs: [size,1], col_idxs: [1,size]
    row_idxs = expand_dims(idxs, 1)  # [size,1]
    col_idxs = expand_dims(idxs, 0)  # [1,size]
    ...
    mask2d = cast(mask2d, float32)  # [size,size]

    # Expand to [1,1,size,size] so it broadcasts over (batch, heads)
    mask = expand_dims(expand_dims(mask2d, 0), 1)
...
def create_causal_mask_(size: int | Tensor) -> Tensor:
    ...
    size = cast(size, int32)
    idxs = range(size)  # [L]
    row = expand_dims(idxs, 1)  # [L,1]
    col = expand_dims(idxs, 0)  # [1,L]
    mask2d = greater(col, row)  # True if j > i
    mask2d = cast(mask2d, float32)  # [L,L]
    return expand_dims(expand_dims(mask2d, 0), 1)  # [1,1,L,L]
...
def combine_masks(
    ...
    *,
    ...
    def _to_bool(x: Tensor) -> Tensor:
        return cast(x, bool_dtype)

    if mask_a is None:
        mb = _to_bool(mask_b)
        return logical_not(mb) if invert_b else mb
    if mask_b is None:
...
#     ones = ones(shape, dtype=float32)

#     # make the [seq_len, seq_len] causal matrix
#     mask2d = 1.0 - linalg.band_part(ones, -1, 0)

#     # now expand batch dim then head dim → [1,1,seq_len,seq_len]
#     mask = expand_dims(mask2d, 0)  # → [1, seq_len, seq_len]
#     mask = expand_dims(mask, 1)    # → [1, 1, seq_len, seq_len]
...
def _create_causal_mask(size: int | Tensor) -> Tensor:
    ...
    mask = 1 - linalg.band_part(ones((size, size)), -1, 0)
    # Add batch and head dimensions for broadcasting
    return mask[
        expand_dims(range(size), 0), :
    ...
    # return expand_dims(expand_dims(
    #     1 - linalg.band_part(ones((size, size)), -1, 0), axis=0), axis=0)
